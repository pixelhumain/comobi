...
import { Organizations,SchemasOrganizationsRest } from '../organizations.js';
import { Projects,SchemasProjectsRest } from '../projects.js';
...
Meteor.methods({
  ...
  insertEvent (doc){
    ...
    event.preferences = doc.preferences;
    /*event.preferences={};
  ...
  updateEvent (modifier,documentId){
    ...
    event.preferences={};
    event.preferences['isOpenData']=modifier["$set"]['preferences.isOpenData'];
    event.preferences['isOpenEdition']=modifier["$set"]['preferences.isOpenEdition'];
  ...
  insertProject (doc){
    ...
    if(doc.startDate){
    project.startDate = doc.startDate;
    }
    if(doc.endDate){
    project.endDate = doc.endDate;
    }
    project.key='project';
    project.collection='projects';
    project.preferences = doc.preferences;
    /*project.preferences={};
    project.preferences['isOpenData']=true;
    project.preferences['isOpenEdition']=true;
    */
    project.address = {};
    project.address['addressCountry'] = doc.country;
    project.address['postalCode'] = doc.postalCode;
    project.address['codeInsee'] = doc.city;
    project.address['addressLocality'] = doc.cityName;
    project.address['regionName'] = doc.regionName;
    project.address['depName'] = doc.depName;
    if(doc.streetAddress){
      project.address['streetAddress'] = doc.streetAddress;
    }
    if(doc.geoPosLatitude && doc.geoPosLongitude){
      project.geo = {};
      project.geo['latitude'] = doc.geoPosLatitude;
      project.geo['longitude'] = doc.geoPosLongitude;
      project.geo['@type'] = 'GeoCoordinates';
      project.geoPosition = {};
      project.geoPosition['type'] = 'Point';
      project.geoPosition['coordinates'] = [parseFloat(doc.geoPosLongitude),parseFloat(doc.geoPosLatitude)];
    }
    console.log(project);

    var retour = apiCommunecter.postPixel("element","save",project);
    return retour;
  },
  updateProject (modifier,documentId){
    ...
    project.name = modifier["$set"].name;

    project.description = modifier["$set"].description;
    if(modifier["$set"].startDate){
    project.startDate = modifier["$set"].startDate;
    }
    if(modifier["$set"].endDate){
    project.endDate = modifier["$set"].endDate;
    }
    project.key='project';
    project.collection='projects';

    project.preferences={};
    project.preferences['isOpenData']=modifier["$set"]['preferences.isOpenData'];
    project.preferences['isOpenEdition']=modifier["$set"]['preferences.isOpenEdition'];

    project.address = {};
    project.address['addressCountry'] = modifier["$set"].country;
    project.address['postalCode'] = modifier["$set"].postalCode;
    project.address['codeInsee'] = modifier["$set"].city;
    project.address['addressLocality'] = modifier["$set"].cityName;
    project.address['regionName'] = modifier["$set"].regionName;
    project.address['depName'] = modifier["$set"].depName;
    if(modifier["$set"].streetAddress){
      project.address['streetAddress'] = modifier["$set"].streetAddress;
    }
    if(modifier["$set"].geoPosLatitude && modifier["$set"].geoPosLongitude){
      project.geo = {};
      project.geo['latitude'] = modifier["$set"].geoPosLatitude;
      project.geo['longitude'] = modifier["$set"].geoPosLongitude;
      project.geo['@type'] = 'GeoCoordinates';
      project.geoPosition = {};
      project.geoPosition['type'] = 'Point';
      project.geoPosition['coordinates'] = [parseFloat(modifier["$set"].geoPosLongitude),parseFloat(modifier["$set"].geoPosLatitude)];
    }
    console.log(project);
  ...
  photoScope (scope,photo,str,idType) {
    ...
    if(retourUpload){
      let documentId= Meteor.call("insertDocumentProfilScope",scope,idType,retourUpload);
      if(documentId){
        return documentId;
      }else{
        throw new Meteor.Error("insertDocument error");
  ...
  insertDocumentProfilScope (scope,idType,retourUpload){
    check(idType, String);
  ...
  insertOrganization (doc){
    ...
    organization.type = doc.type;
    organization.role = doc.role;
    if(doc.email){
    organization.email = doc.email;
    }
    if(doc.url){
    organization.url = doc.url;
    }
    if(doc.tags){
    organization.tags = doc.tags;
    }
    organization.description = doc.description;
    organization.key='organization';
    organization.collection='organizations';
    organization.preferences = doc.preferences;
    /*organization.preferences={};
    organization.preferences['isOpenData']=true;
    organization.preferences['isOpenEdition']=true;
    */
    organization.address = {};
    organization.address['addressCountry'] = doc.country;
    organization.address['postalCode'] = doc.postalCode;
    organization.address['codeInsee'] = doc.city;
    organization.address['addressLocality'] = doc.cityName;
    organization.address['regionName'] = doc.regionName;
    organization.address['depName'] = doc.depName;
    if(doc.streetAddress){
      organization.address['streetAddress'] = doc.streetAddress;
    }
    if(doc.geoPosLatitude && doc.geoPosLongitude){
      organization.geo = {};
      organization.geo['latitude'] = doc.geoPosLatitude;
      organization.geo['longitude'] = doc.geoPosLongitude;
      organization.geo['@type'] = 'GeoCoordinates';
      organization.geoPosition = {};
      organization.geoPosition['type'] = 'Point';
      organization.geoPosition['coordinates'] = [parseFloat(doc.geoPosLongitude),parseFloat(doc.geoPosLatitude)];
    }
    console.log(organization);

    var retour = apiCommunecter.postPixel("element","save",organization);
    return retour;
  },
  updateOrganization (modifier,documentId){
    ...
    organization.name = modifier["$set"].name;
    organization.type = modifier["$set"].type;
    if(modifier["$set"].role){
    organization.role = modifier["$set"].role;
    }
    if(modifier["$set"].email){
    organization.email = modifier["$set"].email;
    }
    if(modifier["$set"].url){
    organization.url = modifier["$set"].url;
    }
    if(modifier["$set"].tags){
    organization.tags = modifier["$set"].tags;
    }
    organization.description = modifier["$set"].description;
    organization.key='organization';
    organization.collection='organizations';
    organization.preferences={};
    organization.preferences['isOpenData']=modifier["$set"]['preferences.isOpenData'];
    organization.preferences['isOpenEdition']=modifier["$set"]['preferences.isOpenEdition'];

    organization.address = {};
    organization.address['addressCountry'] = modifier["$set"].country;
    organization.address['postalCode'] = modifier["$set"].postalCode;
    organization.address['codeInsee'] = modifier["$set"].city;
    organization.address['addressLocality'] = modifier["$set"].cityName;
    organization.address['regionName'] = modifier["$set"].regionName;
    organization.address['depName'] = modifier["$set"].depName;
    if(modifier["$set"].streetAddress){
      organization.address['streetAddress'] = modifier["$set"].streetAddress;
    }
    if(modifier["$set"].geoPosLatitude && modifier["$set"].geoPosLongitude){
      organization.geo = {};
      organization.geo['latitude'] = modifier["$set"].geoPosLatitude;
      organization.geo['longitude'] = modifier["$set"].geoPosLongitude;
      organization.geo['@type'] = 'GeoCoordinates';
      organization.geoPosition = {};
      organization.geoPosition['type'] = 'Point';
      organization.geoPosition['coordinates'] = [parseFloat(modifier["$set"].geoPosLongitude),parseFloat(modifier["$set"].geoPosLatitude)];
    }
    console.log(organization);
  ...
  photoOrganizations (photo,str,idType) {
    check(str, String);
    check(idType, String);
    if (!this.userId) {
      throw new Meteor.Error("not-authorized");
    }
    if (!Organizations.findOne({_id:new Mongo.ObjectID(idType)}).isAdmin()) {
      throw new Meteor.Error("not-authorized");
    }
    let retourUpload = apiCommunecter.postUploadPixel('organizations',idType,'avatar',photo,str);
    if(retourUpload){
      let documentId= Meteor.call("insertDocumentProfilOrganizations",idType,retourUpload);
      if(documentId){
        return documentId;
      }else{
        throw new Meteor.Error("insertDocument error");
  ...
  insertDocumentProfilOrganizations (idType,retourUpload){
    check(idType, String);
    check(retourUpload, Object);
    ...
    let insertDoc = {};
    insertDoc.id = idType;
    insertDoc.type = "organizations";
    insertDoc.folder = "organizations/"+idType;
    insertDoc.moduleId = "communecter";
    insertDoc.author = this.userId;
    insertDoc.doctype = "image";
    insertDoc.name = retourUpload.name;
    ...
    //console.log(doc);
    if(doc){
      Organizations.update({_id:new Mongo.ObjectID(idType)},{$set:{
        'profilImageUrl':'/upload/communecter/organizations/'+idType+'/'+retourUpload.name,
        'profilThumbImageUrl':'/upload/communecter/organizations/'+idType+'/thumb/profil-resized.png?='+new Date + Math.floor((Math.random() * 100) + 1),
        'profilMarkerImageUrl':'/upload/communecter/organizations/'+idType+'/thumb/profil-marker.png?='+new Date + Math.floor((Math.random() * 100) + 1)
      }});
    }
  },
