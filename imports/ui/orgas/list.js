...
//submanager
import { listOrgasSubs } from '../../api/client/subsmanager.js';

let pageSession = new ReactiveDict('pageOrgas');

Template.mapOrga.onCreated(function () {

});

Template.mapOrga.onRendered(function () {
  var self = this;
...
Template.listOrgas.onCreated(function () {
  var self = this;
  ...
  self.autorun(function(c) {
    ...
    if(radius && geo && geo.latitude){
      ...
      let handle = listOrgasSubs.subscribe('citoyenOrgas',latlng,radius);
          self.ready.set(handle.ready());
    }else{
      //console.log('sub list events city');
      let city = Session.get('city');
      if(city && city.geoShape && city.geoShape.coordinates){
        let handle = listOrgasSubs.subscribe('citoyenOrgas',city.geoShape.coordinates);
            self.ready.set(handle.ready());
      }
    }
...
Template.listOrgas.onRendered(function() {
...
Template.listOrgas.helpers({
  orgas () {
    let searchOrgas= pageSession.get('searchOrgas');
    let query={};
    if(searchOrgas){
      if ( searchOrgas.charAt( 0 ) == '#' ) {
        query['name']={$regex : searchOrgas, '$options' : 'i'}
      }else{
        query['name']={$regex : searchOrgas, '$options' : 'i'}
      }

    }
    return Orgas.find(query);
  },
  countOrgas () {
    let searchOrgas= pageSession.get('searchOrgas');
    let query={};
    ...
    return Orgas.find(query).count();
  },
  searchOrgas (){
    return pageSession.get('searchOrgas');
  },
  notificationsCount () {
...
Template.listOrgas.events({
  'keyup #search, change #search': function(orga,template){
    if(orga.currentTarget.value.length>2){
      pageSession.set( 'searchOrgas', orga.currentTarget.value);
    }else{
      pageSession.set( 'searchOrgas', null);
    }
  }
});

Template.orgasAdd.onCreated(function () {
...
Template.orgasEdit.onCreated(function () {
  pageSession.set('error', false );
  pageSession.set('postalCode', null);
  pageSession.set('country', null);
  pageSession.set('city', null);
...
Template.orgasAdd.helpers({
  error () {
    return pageSession.get( 'error' );
  }
});

Template.orgasEdit.helpers({
  event () {
...
Template.orgasFields.onRendered(function() {
  var self = this;
  pageSession.set('postalCode', null);
  pageSession.set('country', null);
  pageSession.set('city', null);
